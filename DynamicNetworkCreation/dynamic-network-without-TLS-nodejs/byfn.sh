...
function printHelp () {
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"whochannel\")"
  ...
  echo
  echo "	byfn.sh generate -c whochannel"
  echo "	byfn.sh up -c whochannel -s couchdb"
  echo "        byfn.sh up -c whochannel -s couchdb -i 1.1.0-alpha"
  echo "	byfn.sh up -l node"
  echo "	byfn.sh down -c whochannel"
  echo "        byfn.sh upgrade -c whochannel"
  echo
...
function networkUp () {
  ...
  docker exec whocli scripts/script.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT
  if [ $? -ne 0 ]; then
...
function upgradeNetwork () {
  ...
  docker-compose $COMPOSE_FILES stop whocli
  docker-compose $COMPOSE_FILES up -d --no-deps whocli
  ...
  for PEER in peer0.who.example.com peer1.who.example.com peer0.org2.example.com peer1.org2.example.com; do
  ...
  docker exec whocli scripts/upgrade_to_v11.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Test failed"
    exit 1
  fi
...
function replacePrivateKey () {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/who.example.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# and two Peer Orgs (``who`` & ``Org2``) each managing and maintaining two peer nodes.
...
# (``peer0.who.example.com`` & ``peer0.org2.example.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile whoChannel -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID $CHANNEL_NAME
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for whoMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile whoChannel -outputAnchorPeersUpdate ./channel-artifacts/whoMSPanchors.tx -channelID $CHANNEL_NAME -asOrg whoMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
...
CLI_DELAY=3
# channel name defaults to "whochannel"
CHANNEL_NAME="whochannel"
